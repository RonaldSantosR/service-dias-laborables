...
//	Iterable<Feriado> findAllByAmbitoid2(Long id);	
	...
	@Query(value=" Select f.fecha From feriado As f where f.feriado_id in (select a.feriado_id From feriado_ambito as a where a.ambito_id=?1) and f.tipo_periodo_id in (select tf.tipo_periodo_id from tipo_periodo as tf where tf.tipo_periodo_id=1) ", nativeQuery=true)	
	Iterable<Date> findporAmbito(long id);

	@Query(value=" Select f.fecha From feriado As f where f.feriado_id in (select a.feriado_id From feriado_ambito as a where a.ambito_id=?1) and f.tipo_periodo_id in (select tf.tipo_periodo_id from tipo_periodo as tf where tf.tipo_periodo_id=2)", nativeQuery=true)	
	...
	@Query(value=" Select * From feriado As f where f.feriado_id in (select a.feriado_id From feriado_ambito as a where a.ambito_id=?2) and cast(f.fecha as date)=?1 ", nativeQuery=true)	
